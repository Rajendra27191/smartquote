angular
		...
		.controller(
				'SQPaymentReminderController',
				function($window, $scope, $rootScope, $log, $state, $timeout,
						$http, $upload, SQPaymentReminderFactory, $filter) {
					...
					$scope.onExcelSelect = function($files) {
						...
						for (var i = 0; i < $files.length; i++) {
							if (($files[i].name.split('.').pop() == 'xlsx')) {
								...
								// console.log('Please upload valid excel
								// file.');
								...
								.success(
										function(data, status, header, config) {
											if (data.code.toUpperCase() == "SUCCESS") {
												...
												document
														.getElementById('emailTemplate').value = '';
												latestExcelFile = {};
												$scope.lastCall = 'load';
					...
					$scope.uploadFile = function() {
						if (latestExcelFile === undefined
							...
							if ((latestExcelFile.size / 1024) < 15360) {// 6144
								...
								$rootScope
										.alertError("File size must ne less than 15MB");
							}
						}
					};
					// ======================================
					...
					var cleanupEventGetFileListDone = $scope.$on(
							"GetFileListDone", function(event, message) {
								$scope.handleGetFileListDoneResponse(message);
							});

					var cleanupEventGetFileListNotDone = $scope.$on(
							"GetFileListNotDone", function(event, message) {
					...
					var cleanupEventUnloadFileDone = $scope.$on(
							"UnloadFileDone", function(event, message) {
								$scope.handleUnloadFileDoneResponse(message);
							});

					var cleanupEventUnloadFileNotDone = $scope.$on(
							"UnloadFileNotDone", function(event, message) {
								$rootScope.alertServerError("Server error");
								$rootScope.hideSpinner();
							});
					...
					var cleanupEventGetFileDetailListDone = $scope
							.$on(
									"GetFileDetailListDone",
									function(event, message) {
										$scope
												.handleGetFileDetailListDoneResponse(message);
									});

					var cleanupEventGetFileDetailListNotDone = $scope.$on(
							"GetFileDetailListNotDone",
							function(event, message) {
								$rootScope.alertServerError("Server error");
								$rootScope.hideSpinner();
							});
					...
					$scope.getCustomerDetailFromFile = function() {
						...
						if ($scope.duration) {
							if ($scope.objFile) {
								...
								$rootScope.showSpinner();
								SQPaymentReminderFactory
										.GetCustomerDetailFromFile(
												$scope.objFile.fileName,
												$scope.objFile.fileId);
							}
							;
						} else {
							$scope.invalidDuration = true;
					...
					$scope.handleGetCustomerDetailFromFileDoneResponse = function(
							data) {
						$scope.productDetails = {};
						if (data) {
							if (data.code) {
								if (data.code.toUpperCase() == 'SUCCESS') {
									$scope.customerDetailListTemp = angular
											.copy(data.fileLogList);
									;
									$scope.customerDetailList = angular
											.copy(data.fileLogList);
									;
					...
					var cleanupEventGetCustomerDetailFromFileDone = $scope
							.$on(
									"GetCustomerDetailFromFileDone",
									function(event, message) {
										$scope
												.handleGetCustomerDetailFromFileDoneResponse(message);
									});

					var cleanupEventGetCustomerDetailFromFileNotDone = $scope
							.$on("GetCustomerDetailFromFileNotDone", function(
									event, message) {
								$rootScope.alertServerError("Server error");
								$rootScope.hideSpinner();
							});
						...
						if (invalid) {
							...
							if (customer) {
								if (customer.email == ""
									...
									$rootScope.showSpinner();
									SQPaymentReminderFactory.ChangeEmailId(
											customer.customerCode,
											customer.fileId, customer.email);
								}
							}
							;
						}
					...
					var cleanupEventChangeEmailIdDone = $scope
							.$on(
									"ChangeEmailIdDone",
									function(event, message) {
										$scope
												.handleChangeEmailIdDoneResponse(message);
									});

					var cleanupEventChangeEmailIdNotDone = $scope.$on(
							"ChangeEmailIdNotDone", function(event, message) {
								$rootScope.alertServerError("Server error");
								$rootScope.hideSpinner();
							});
					// ==================================
					...
					$scope.stop = function(list, index) {
						...
						customerEmail = angular
								.copy($scope.customerDetailListTemp[index]);
					...
					$scope.updateEmail = function(customer, index) {
						...
						// console.log(customer);
						if (customer.email) {
							$scope.customerDetailListTemp[index] = angular
									.copy(customer);
							$scope.editing[index] = false;
						}
					}
					...
					$scope.toggleAll = function() {
						...
						angular.forEach($scope.customerDetailList,
								function(itm) {
					...
					$scope.getAllSelectedRows = function() {
						$scope.selectedRows = [];
						var selectedRows = $filter("filter")(
								$scope.customerDetailList, {
									selected : true
								}, true);
					...
					$scope.compose = null;
					// $scope.emailConfigList=[{email: 'test@gmail.com', id: 30
					// },{ email: 'twotest@gmail.com', id: 27 },{ email:
					// 'threextest@gmail.com', id: 50 }];
					...
					$scope.assignEmailFormat = function(data) {
						...
						$scope.compose = {}
						$scope.emailConfigList = angular
								.copy(data.emailFormatList);
						$scope.compose.from = "";
					...
					$scope.handleGetEmailFormatTemplateDoneResponse = function(
							data) {
						$scope.productDetails = {};
						if (data) {
							if (data.code) {
					...
					var cleanupEventGetEmailFormatTemplateDone = $scope
							.$on(
									"GetEmailFormatTemplateDone",
									function(event, message) {
										$scope
												.handleGetEmailFormatTemplateDoneResponse(message);
									});

					var cleanupEventGetEmailFormatTemplateNotDone = $scope.$on(
							"GetEmailFormatTemplateNotDone", function(event,
									message) {
								$rootScope.alertServerError("Server error");
								$rootScope.hideSpinner();
							});
					...
					$scope.inputReminderChanged = function(reminder, from) {
						...
						if (reminder) {
							angular.forEach($scope.emailConfigList, function(
									value, key) {
								if (reminder == value.templateId
										&& from.configId == value.configId) {
					...
					var cleanupEventSendReminderDone = $scope.$on(
							"SendReminderDone", function(event, message) {
								$scope.handleSendReminderDoneResponse(message);
							});

					var cleanupEventSendReminderNotDone = $scope.$on(
							"SendReminderNotDone", function(event, message) {
								$rootScope.alertServerError("Server error");
								$rootScope.hideSpinner();
							});
					...
					var cleanupEventGetEmailLogListDone = $scope
							.$on(
									"GetEmailLogListDone",
									function(event, message) {
										$scope
												.handleGetEmailLogListDoneResponse(message);
									});

					var cleanupEventGetEmailLogListNotDone = $scope.$on(
							"GetEmailLogListNotDone", function(event, message) {
								$rootScope.alertServerError("Server error");
								$rootScope.hideSpinner();
							});
					// ===================================
					...
					$scope.getEmailLogDetails = function(batchId, status) {
						$rootScope.showSpinner();
						SQPaymentReminderFactory.GetEmailLogDetails(batchId,
								status);
					};
							...
							if (data.code) {
								if (data.code.toUpperCase() == 'SUCCESS') {
									...
									if ($scope.emailLogDetailList.length > 0) {
										...
										$rootScope
												.alertError("Sorry no record found");
									}
									;
								} else {
									$rootScope.alertError(data.message);
					...
					var cleanupEventGetEmailLogDetailsDone = $scope
							.$on(
									"GetEmailLogDetailsDone",
									function(event, message) {
										$scope
												.handleGetEmailLogDetailsDoneResponse(message);
									});

					var cleanupEventGetEmailLogDetailsNotDone = $scope.$on(
							"GetEmailLogDetailsNotDone", function(event,
									message) {
								$rootScope.alertServerError("Server error");
								$rootScope.hideSpinner();
							});
					...
					var cleanupEventResendReminderDone = $scope
							.$on(
									"ResendReminderDone",
									function(event, message) {
										$scope
												.handleResendReminderDoneResponse(message);
									});

					var cleanupEventResendReminderNotDone = $scope.$on(
							"ResendReminderNotDone", function(event, message) {
								$rootScope.alertServerError("Server error");
								$rootScope.hideSpinner();
							});
					...
					$scope.saveAsExcel = function(batchId, status) {
						var url = $rootScope.projectName
								+ "/saveEmailLogAsExcel?batchId=" + batchId
								+ "&status=" + status;
						window
								.open(url,
										'location=1,status=1,scrollbars=1,width=1050,fullscreen=yes,height=1400');
					};
					$scope.saveAsExcelClicked = function(emailLogData) {
						// console.log(emailLogData)
						if (emailLogData) {
							$scope.saveAsExcel(emailLogData.batchId,
									emailLogData.status);
						}
					};
					// =======================================================
					...
					$scope.handleGetPendingEmailListDoneResponse = function(
							data) {
						if (data) {
							if (data.code) {
								if (data.code.toUpperCase() == 'SUCCESS') {
					...
					var cleanupEventGetPendingEmailListDone = $scope
							.$on(
									"GetPendingEmailListDone",
									function(event, message) {
										$scope
												.handleGetPendingEmailListDoneResponse(message);
									});

					var cleanupEventGetPendingEmailListNotDone = $scope.$on(
							"GetPendingEmailListNotDone", function(event,
									message) {
								$rootScope.alertServerError("Server error");
								$rootScope.hideSpinner();
							});
					...
					$scope.abortMailBtnClicked = function() {
						...
						if ($scope.selectedRowsForAbort.length > 0) {
							// console.log($scope.jsonForSendMail());
							// $rootScope.showSpinner();
							SQPaymentReminderFactory.AbortEmail($scope
									.jsonForAbortMail());
						} else {
							$rootScope
									.alertError("Please Select Mail To Abort")
						}
					};
					...
					var cleanupEventAbortEmailDone = $scope.$on(
							"AbortEmailDone", function(event, message) {
								$scope.handleAbortEmailDoneResponse(message);
							});

					var cleanupEventAbortEmailNotDone = $scope.$on(
							"AbortEmailNotDone", function(event, message) {
								$rootScope.alertServerError("Server error");
								$rootScope.hideSpinner();
							});
					// =======================================================
										...
										function(data, status, header, config) {
											if (data.code.toUpperCase() == "SUCCESS") {
												$rootScope
														.alertSuccess(data.message);
												document
														.getElementById('emailFile').value = '';
												latestEmailExcelFile = {};
												// $scope.lastCall='load';
											} else if (data.code == "sessionTimeOut") {
												$rootScope.$broadcast(
					...
					$scope.uploadEmailFile = function() {
						if (latestEmailExcelFile === undefined
								|| latestEmailExcelFile === {}
								|| document.getElementById('emailFile').value === '') {
