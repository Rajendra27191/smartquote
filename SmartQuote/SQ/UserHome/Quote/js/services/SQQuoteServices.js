angular.module('sq.SmartQuoteDesktop')
  .factory('SQQuoteServices', [
    ...
    function ($rootScope, $resource, $http, $state, $log) {
      ...
      quote.AddProductToProposal = function (objProductBean) {
        console.log("addProductToProposal")
        // console.log(objProductBean);
        data = $.param({ objProductBean: objProductBean });
        $http.post($rootScope.projectName + '/addProductToProposal', data, config)
          .success(function (data, status, headers, config) {
            // console.log(data);
            if (data.code == "sessionTimeOut") {
              // $rootScope.$broadcast('QuoteSessionTimeOut', data);     
            } else {
              $rootScope.$broadcast('AddProductToProposalDone', data);
            }
          })
          .error(function (data, status, header, config) {
            console.log(data);
            $rootScope.$broadcast('AddProductToProposalNotDone', data);
          });
      };

      quote.EditProductIntoProposal = function (objProductBean) {
        console.log("addProductToProposal")
        // console.log(objProductBean);
        data = $.param({ objProductBean: objProductBean });
        $http.post($rootScope.projectName + '/editProductIntoProposal', data, config)
          .success(function (data, status, headers, config) {
            // console.log(data);
            if (data.code == "sessionTimeOut") {
              // $rootScope.$broadcast('QuoteSessionTimeOut', data);     
            } else {
              $rootScope.$broadcast('EditProductIntoProposalDone', data);
            }
          })
          .error(function (data, status, header, config) {
            console.log(data);
            $rootScope.$broadcast('EditProductIntoProposalNotDone', data);
          });
      }

      quote.DeleteProductFromProposal = function (objProductBean) {
        console.log("DeleteProductFromProposal")
        // console.log(objProductBean);
        data = $.param({ objProductBean: objProductBean });
        $http.post($rootScope.projectName + '/deleteProductFromProposal', data, config)
          .success(function (data, status, headers, config) {
            console.log(data);
            if (data.code == "sessionTimeOut") {
              // $rootScope.$broadcast('QuoteSessionTimeOut', data);     
            } else {
              $rootScope.$broadcast('DeleteProductFromProposalDone', data);
            }
          })
          .error(function (data, status, header, config) {
            console.log(data);
            $rootScope.$broadcast('DeleteProductFromProposalNotDone', data);
          });
      }

      quote.SaveGeneratedProposal = function (objQuoteBean) {
        console.log("SaveGeneratedProposal");
        console.log(objQuoteBean);
        data = $.param({ objQuoteBean: objQuoteBean });
        $http.post($rootScope.projectName + '/saveGeneratedProposal', data, config)
          .success(function (data, status, headers, config) {
            console.log(data);
            if (data.code == "sessionTimeOut") {
              // $rootScope.$broadcast('QuoteSessionTimeOut', data);     
            } else {
              $rootScope.$broadcast('SaveGeneratedProposalDone', data);
            }
          })
          .error(function (data, status, header, config) {
            console.log(data);
      ...
      quote.GetGeneratedQuoteView = function (quoteId) {
        ...
        QuoteServices.getGeneratedQuoteViewAPI.getGeneratedQuoteViewMethod({ "quoteId": quoteId },function (success) {
          if (success.code == "sessionTimeOut") {
            ...
            $rootScope.$broadcast('GetGeneratedQuoteViewDone', success);
          }
        }, function (error) {
          console.log(error);
