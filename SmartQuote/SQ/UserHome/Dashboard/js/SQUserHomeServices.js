  ...
  .factory('SQUserHomeServices', [
    ...
    function ($rootScope, $resource, $http, $state, $log) {
      var UserHomeServices = {
        ...
        refreshProductFileApi: $resource($rootScope.projectName + '/refreshProductFile', {}, { refreshProductFileMethod: { method: 'GET' } })
      ...
      userhome.RefreshProductFile = function () {
        UserHomeServices.refreshProductFileApi.refreshProductFileMethod(function (success) {
          console.log(success)
          if (success.code == "sessionTimeOut") {
            $rootScope.$broadcast('SessionTimeOut', success);
          } else {
            $rootScope.$broadcast('RefreshProductFileDone', success);
          }
        }, function (error) {
          console.log(error);
          $rootScope.$broadcast('RefreshProductFileNotDone', error);
        });
      };
